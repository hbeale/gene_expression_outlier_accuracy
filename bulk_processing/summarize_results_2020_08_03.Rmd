...
split_by_quartile <- function(bulk_results, col_name_fn = Expression, col_name_text = "Expression", this_suffix = " Log2(TPM+1)"){
  # Upper quartile
 bulk_results_bit <- lapply(c("Up", "Down"), function(this_type){
   these_bulk_results <- bulk_results %>% filter(Type == this_type)
    this_threshold <- these_bulk_results %>% summarize(this_q = quantile({{col_name_fn}}, 0.75)) %>% pull(this_q) %>% unique
    bulk_results_upper_bit <- these_bulk_results %>% 
      filter({{col_name_fn}} > this_threshold) %>%
      mutate(set = paste0(col_name_text,  "_UQ"),
             set_type_label = paste0(">", round(this_threshold, 1), this_suffix, ", n=", label_comma()(n())))
  # Lower quartile
    this_threshold <- these_bulk_results %>% summarize(this_q = quantile({{col_name_fn}}, 0.25)) %>% pull(this_q) %>% unique
    bulk_results_lower_bit <- these_bulk_results %>% 
      filter({{col_name_fn}} < this_threshold) %>%
#      mutate(set = paste(col_name_text, "<", round(this_threshold, 1), this_suffix))
      mutate(set = paste0(col_name_text, "_LQ"),
             set_type_label = paste0("<", round(this_threshold, 1), this_suffix, ", n=", label_comma()(n())))
...
# quantile(bulk_results[["MEND_depth"]], 0.75) # works
# bulk_results %>% ungroup %>% quantile(.data[["MEND_depth"]], 0.75) # fails
# bulk_results %>% quantile(MEND_depth, 0.75) # fails
# 
# enquo(value)
...
split_by_quartile(bulk_results_for_fig1, col_name = Expression, col_name_text="Expression", this_suffix = " Log2(TPM+1)"),
split_by_quartile(bulk_results_for_fig1, col_name = Depth, col_name_text="Depth", this_suffix = "M MEND reads"),
split_by_quartile(bulk_results_for_fig1, col_name = Length, col_name_text="Length", this_suffix = "kb"),
split_by_quartile(bulk_results_for_fig1, col_name = Difference, col_name_text="Difference", this_suffix = "%"),
)
...
all_boxplots <- ggplot(figure1_data_v2, aes(y=set, x=Mean_Accuracy, fill = Type)) +
  ...
  geom_text(data = boxplot_n_labels, 
            aes(x=20, y=set, label = set_type_label), hjust = 0, vjust = -1, size = 3) +
  theme_bw() +
...
```{r figure-1f, fig.height=8, fig.width=6}
